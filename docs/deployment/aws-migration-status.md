...
- Configuration backups: `configuration/20260908-initial.tar.gz` (operator recovery key) and `configuration/20260908-final-host.tar.gz` (protected host files, runtime, restricted tunnel configuration).
...
- Verified CI deployment: run `34173426446`, release `2f18b0f2d0e5-34173426446-1`; later releases are recorded by CI and `/var/lib/mwf/current-release.json`.
...
Use [the AWS runbook](../../infra/aws/README.md). The Render role freeze must be reset explicitly before a pre-write rollback. After AWS receives new writes, copy the latest data back before routing to Render, or repair AWS in place. App rollback does not reverse database migrations. Preserve the final Render backup for its full 30-day retention.
